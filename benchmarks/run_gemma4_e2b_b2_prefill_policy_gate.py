#!/usr/bin/env python3
"""Loaded-model B2/P2048 prefill policy gate for Gemma 4 E2B on L4.

The production B4 and B8 dispatches remain untouched. The gate loads one model
and measures complete prefill/first-token latency for the generic baseline,
isolated full/sliding attribution, and several B2-only combined launch
geometries. A candidate is eligible only with exact layer hits, stable timings,
no kernel errors, and one real generated-token digest across every case.
"""
...
from benchmarks.run_gemma4_e2b_b4_prefill_policy_gate import (
    _attention_modules,
    _hits,
    _token_digest,
)
...
CASES: tuple[dict[str, Any], ...] = (
    ...
    {
        "name": "sliding_h256_only_g1_bm16_bn64_w4_s2",
        "full": False,
        "sliding": True,
        "config": (1, 16, 64, 4, 2),
    },
    {
        "name": "combined_g1_bm16_bn32_w4_s2",
        "full": True,
        "sliding": True,
        "config": (1, 16, 32, 4, 2),
    },
    ...
    {
        "name": "combined_g2_bm8_bn64_w8_s2",
        "full": True,
        "sliding": True,
        "config": (2, 8, 64, 8, 2),
    },
    ...
    {
        "name": "combined_g1_bm16_bn128_w4_s2",
        "full": True,
        "sliding": True,
        "config": (1, 16, 128, 4, 2),
    },
...
def _set_case(
    ...
    os.environ.update(
        {
            TILE_PREFIX + "GROUP_HEADS": str(group_heads),
            TILE_PREFIX + "BLOCK_M": str(block_m),
            TILE_PREFIX + "BLOCK_N": str(block_n),
            TILE_PREFIX + "NUM_WARPS": str(warps),
            TILE_PREFIX + "NUM_STAGES": str(stages),
        }
    )
...
def summarize(
    ...
    *,
    ...
    token_digest_exact = len(all_digests) == 1
    ...
    return {
        ...
        "token_digest_exact": token_digest_exact,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prompts, actual_tokens = matrix.build_prompts(
        engine.tokenizer,
        BATCH_SIZE,
        PROMPT_TOKENS,
    )
