...
RUN_ID="${RUN_ID:-gemma4_e2b_b2_prefill_$(date -u +%Y%m%dT%H%M%SZ)}"
OUT="${OUT:-$REPO/bench_results/gemma4_e2b_b2_prefill/$RUN_ID}"
mkdir -p "$OUT"

python benchmarks/run_gemma4_e2b_b2_prefill_policy_gate.py \
  --model "$MODEL" \
  --warmups "${WARMUPS:-1}" \
  --repeats "${REPEATS:-3}" \
